...
if ! command -v curl >/dev/null 2>&1; then
	echo "curl is required to fetch marketplace metadata." >&2
	exit 1
fi
...
if ! installed_pair="$(get_installed_version "$INSTALL_ROOT" "$PLUGIN_NAME" "$OWNER" "$REPO")"; then
	echo "Cannot find local installed plugin version for '$PLUGIN_NAME' under '$INSTALL_ROOT'." >&2
	exit 1
fi
...
if ! curl -fsSL --max-time 20 "$remote_url" -o "$tmp_marketplace"; then
	echo "Failed to fetch marketplace: $remote_url" >&2
...
if [[ -z "$latest_version" ]]; then
	echo "Plugin '$PLUGIN_NAME' not found in marketplace: $remote_url" >&2
	exit 1
fi
...
echo "Installed source : $installed_source"
echo "Marketplace URL  : $remote_url"
echo "Latest version   : $latest_version"

if [[ "$cmp" == "-1" ]]; then
	echo
	echo "Update available: $local_version -> $latest_version"
	echo "Run: apm update $PLUGIN_NAME"
	exit 2
fi
...
echo
echo "Local version ($local_version) is newer than marketplace ($latest_version)."
exit 0
